...
namespace DTOMaker.SrcGen.MemBlocks
{
    ...
    public class MemBlocksSourceGenerator : SourceGeneratorBase
    {
        ...
        protected override void OnExecute(GeneratorExecutionContext context)
        {
            ...
            foreach (var entity in domain.ClosedEntities.Values.OfType<MemBlockEntity>())
            {
            ...
            // emit each entity
            foreach (var entity in domain.ClosedEntities.Values.OrderBy(e => e.TFN.FullName).OfType<MemBlockEntity>())
            {
